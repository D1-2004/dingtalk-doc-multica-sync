...
- **容忍重序列化的对比,但反解只在散文做**:代码块/行内代码里逐字保留 —— 钉钉在代码里用 HTML 实体转义(另行反解到底、不设 5 次上限),**不用反斜杠**;所以代码里的 `\.` `\|` `\$`(regex/shell)是真实内容,绝不当 adoc 转义去掉,否则会把真改动误判成一致、漏写回。归一化保证幂等(pull 与 compare 两次不漂)。`_斜体_`↔`*斜体*` 判一致免写,`user_id→userid`/代码里 `\.→.` 判真改动照写。
- **乐观锁 + 回读**:带 revision 写,写后回读正文 + JSONML + revision+1 三重校验;初读后钉钉侧又变了则判 `conflict` 拒绝覆盖(防盖并发编辑)。大文档写后 revision 偶发未落,读侧退避重试。
- **限流退避**:`doc read` 有函数级限流,批量读命中 `函数触发限流` 自动退避重试,不当漂移误报。
- **已知钉钉限制(fail-closed)**:**标题里的行内代码**(`# 用 \`x\` 判定`)会被钉钉整个吞掉、无法保真往返 —— 这类内容渲染回读不匹配时**拒绝写入**(宁可报失败,不写坏)。把行内代码挪出标题即可。
